...
export class OpenAISTT implements ISTTService {
  ...
  async transcribe(base64Audio: string): Promise<SttResult> {
    ...
    console.log(`[OpenAI Whisper STT REST] Transcribed: "${text}" (${languageCode})`);
  ...
  async handleAudioChunk(sessionId: string, base64Chunk: string): Promise<void> {
    ...
    if (!session) {
      console.log(`[OpenAI Whisper STT Buffering] Starting buffering session: ${sessionId}`);
    ...
    try {
      ...
      console.log(`[OpenAI Whisper STT Buffering] Chunk size: ${buf.length} bytes, Max Amplitude: ${maxAmp.toFixed(4)}`);
      ...
      if (maxAmp > threshold && !session.speechDetected) {
        ...
        console.log(`[OpenAI Whisper STT Buffering] Speech detected for session ${sessionId} (barge-in triggered with threshold ${threshold})`);
        this.speechDetectedCallbacks.forEach(cb => cb(sessionId));
      }
    } catch (err) {
      console.warn("[OpenAI Whisper STT Buffering] Failed to calculate amplitude:", err);
    }
  }
  ...
  async handleAudioEnd(sessionId: string): Promise<SttResult> {
    ...
    console.log(`[OpenAI Whisper STT Buffering] Finalizing audio for ${sessionId}. Total size: ${totalLength} bytes.`);
    ...
    console.log(`[OpenAI Whisper STT Buffering] Final Transcription: "${text}" (${languageCode})`);
    return { text, languageCode, confidence };
  }
  ...
  cleanupSession(sessionId: string): void {
    ...
    if (deleted) {
      console.log(`[OpenAI Whisper STT] [Session: ${sessionId}] STT session state cleaned up.`);
    }
  }
}
