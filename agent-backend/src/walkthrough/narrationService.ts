...
register(
  ...
  "orders_customer_selection_1.wav"
);
register(
...
register(
  ...
  "orders_subform_rentalItem_introduction.wav"
);
register(
  "What product is being rented out?",
  "orders_subform_rentalItem_product.wav"
);
register(
  "How many are they renting?",
  "orders_subform_rentalItem_quantity.wav"
);
register(
  "How long are they renting this for? Enter the duration in days.",
  "orders_subform_rentalitem_rentalduraiton_1.wav"
);
...
export class NarrationService {
  ...
  async speak(
    ...
    if (audioFilename) {
      ...
      try {
        const filePath = join(__dirname, "../../voicefiles_wav", audioFilename);
        const buffer = await fsPromises.readFile(filePath);

        // Strip the 44-byte standard WAV header to get raw 16-bit PCM bytes
        const rawPCM = buffer.length > 44 ? buffer.subarray(44) : buffer;
